variable "function_apps" {
  type = map(object({
    ...
    fc1_runtime_version           = optional(string, "10.0")            # Default .NET 10
...
variable "function_app_environment_variables" {
  description = "A map of environment variables to set for each Function App."
  type        = map(map(string))
  default     = {}
}